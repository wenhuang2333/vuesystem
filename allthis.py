...
app = Flask(__name__, static_folder="./first_dist", static_url_path="/")
CORS(app)
...
@app.route('/api/getsound',methods=['POST'])
def get_soundfile():
    data = request.json
    date_str = data.get("date")  # 前端传过来的 "20250917" 之类的字符串
...
def serve_vue(path):
    if path != "" and os.path.exists(os.path.join(app.static_folder, path)):
        # 如果访问的是已有的静态资源文件（比如 js/css）
        return send_from_directory(app.static_folder, path)
    else:
        # 否则一律返回 index.html，让 Vue Router 接管
